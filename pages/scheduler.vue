  ...
  <div>
      ...
      <nav class="-mb-px flex space-x-8" aria-label="Tabs">
        <button
          ...
          :class="[
            ...
            'whitespace-nowrap py-3 px-1 border-b-2 font-medium text-lg transition-colors duration-200'
          ]"
        >
          Jobs
        </button>
    ...
    <div class="space-y-8">
      ...
      <div v-if="selectedTab === 'jobs'">
        <!-- Jobs List Card -->
        <div class="bg-card rounded-lg shadow-md border border-border-color overflow-hidden transition-all duration-300 ease-in-out hover:shadow-lg hover:border-primary hover:scale-[1.005] mb-8">
          <div class="p-6">
            ...
            <div v-if="isJobsOpen" class="overflow-x-auto mt-4">
              <div class="mb-4 flex gap-4">
                ...
                >
                  ...
                  <option value="active">Aktif ({{ activeJobsCount }})</option>
                  <option value="inactive">Pasif ({{ inactiveJobsCount }})</option>
                </select>
              </div>
              <table class="w-full text-left">
                ...
                <tbody>
                  ...
                  >
                    ...
                    <td class="p-4">
                      <div class="mb-1">
                        <StatusBadge type="active" :label="`${job.jobInstances.filter(i => i.enable).length} Aktif`" />
                      </div>
                      <div>
                        <StatusBadge type="inactive" :label="`${job.jobInstances.filter(i => !i.enable).length} Pasif`" />
                      </div>
                    </td>
      ...
      <div v-if="selectedTab === 'job-instances'">
        <!-- Job Instances Card -->
        <div class="bg-card rounded-lg shadow-md border border-border-color overflow-hidden transition-all duration-300 ease-in-out hover:shadow-lg hover:border-primary hover:scale-[1.005]">
          <div class="p-6">
            ...
            <div v-if="isInstancesOpen" class="overflow-x-auto mt-4">
              <div class="mb-4 flex gap-4 items-center">
                <label for="project-select-instances" class="text-text-primary font-medium">Proje Seç:</label>
                <select
              ...
              <div class="mb-4 flex gap-4 items-center" v-if="selectedProjectForInstances">
                <label for="job-select-instances" class="text-text-primary font-medium">Job Seç:</label>
                <select
                  id="job-select-instances"
                  v-model="selectedJobForInstances"
                  class="px-4 py-2 border border-border-color rounded-lg bg-input-bg text-text-primary shadow-sm focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary transition-all duration-200 flex-grow"
                >
                  <option :value="null">-- Tüm Joblar --</option>
              ...
              <div v-if="selectedJobForInstances">
                <div class="mb-4 flex gap-4">
                  ...
                  >
                    <option value="">Tüm Durumlar</option>
                    <option value="active">Aktif ({{ activeInstancesCount }})</option>
                    <option value="inactive">Pasif ({{ inactiveInstancesCount }})</option>
                  </select>
                </div>
        ...
        <div class="bg-card rounded-lg shadow-md border border-border-color overflow-hidden transition-all duration-300 ease-in-out hover:shadow-lg hover:border-primary hover:scale-[1.005]">
          <div class="p-6">
            ...
            <div v-if="isLogsOpen" class="overflow-x-auto max-h-96 mt-4">
              <div class="mb-4 flex gap-4 items-center">
                <label for="project-select-logs" class="text-text-primary font-medium">Proje Seç:</label>
                <select
              ...
              <div class="mb-4 flex gap-4 items-center" v-if="selectedProjectForLogs">
                <label for="job-select-logs" class="text-text-primary font-medium">Job Seç:</label>
                <select
                  id="job-select-logs"
              ...
              <div v-if="selectedJobForLogs">
                <div class="mb-4 flex gap-4">
                  ...
                  >
                    <option value="">Tüm Durumlar</option>
                    <option value="success">Başarılı ({{ successLogsCount }})</option>
                    <option value="failure">Başarısız ({{ failureLogsCount }})</option>
                  </select>
                </div>
...
const handleInstanceUpdated = (updatedInstance) => {
  ...
  if (!selectedJobForInstances.value) return;
  const job = allJobsData.value.find(j => j.id === selectedJobForInstances.value.id); // allJobsData'da bul
  if (job) {
...
const promptStartAllInstances = (job) => {
  confirmationConfig.value = {
    title: 'Tüm Instance\'ları Başlat',
    message: `'${job.url}' adresine sahip job'ın tüm pasif instance'larını başlatmak istediğinizden emin misiniz?`,
    confirmText: 'Tümünü Başlat',
    confirmButtonClass: 'bg-green-600 hover:bg-green-700',
...
// Watchers for job selection in instances and logs tabs to reset their filters and pagination
watch(selectedJobForInstances, () => {
  instancesCurrentPage.value = 1;
  instancesFilterKey.value = '';
  instancesFilterStatus.value = '';
});

watch(selectedJobForLogs, () => {
  logsCurrentPage.value = 1;
  logsFilterJobKey.value = '';
  logsFilterStatus.value = '';
});
